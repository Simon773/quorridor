#ligne 164 à modifier dans le nom de la fonction
#QUESTIONS
...
class QuorridorPresenter:
    ...
    def lien_model(self,nb):
        if nb==1: #choix des unites
            ...
            self.model.Set_lien_presenter(self.enregistrer_equipe,12)
    def enregistrer_equipe(self,player):
        nom =self.view.nom_equipe()
        # type de pion 0:pionSappeur, 1:pion Jumper, 2:pion sprinter
        if player.type=="S":
            choix_pion =0
        elif player.type=="J":
            ...
            choix_pion=2
        self.client.registerTeam(nom,choix_pion)
        return nom
    ...
    def recuperer_action(self,player):
        print(self.bot)
        if self.bot:
            mouvement= self.model.choix_action_aleatoire(player)
        else:
