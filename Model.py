...
class QuorridorModel:
    ...
    def dijkstra_modifie( self,destination, depart):
        ...
        #retrouver le chemin le plus rapide
        chemin = []
    ...
    def initialiser_jeu(self):
        ...
        while not victoire:
            ...
            action= self.choix_action(self.current_player)
            if action ==1:
            ...
            if self.check_win(self.current_player):
                ...
                break
            self.gestion_mur_temporaire()
            self.demander_priorite()
            tour = (tour + 1) % 2
            self.changer_joueur(tour)

    def initialiser_jeu_server(self):
        player=self.joueur
        unit_type =self.choix_unite()
        self.attribuer_type_unite(self.joueur, unit_type)
        nom=self.demander_nom_equipe(player)
        player.nom_joueur=nom

        #avant d'initialiser les positions et le plateau, il faut avoir son choix d'unité
        #par soucis d'éfficacité, il peut être intéressant de connaitre de faire une requete avant au serveur
        self.init_pos()
        self.initialiser_plateau()
        self.actualiser_vue()

        #variable qui sera bien à mettre à jour mais differement avec notamment un retour du serveur (fonction)
        for player in [self.joueur,self.adversaire]:
            player.murs_poss=self.choix_mur(player)
    ...
    def choix_action_dijkstra(self,player):
        if player == self.joueur:
            destination =0
        else:
            destionation =self.taille-1
        depart=(player.pos_x,player.pos_y)
        distance =self.dijkstra_modifie(destination,player)
    def attribuer_type_unite(self, joueur, unit_type):
        pass
    ...
    def choix_placement_mur(self,player):
        pass
    def demander_nom_equipe(self,player):
        pass
    def choix_action(self,player):
        pass
    ...
    def Set_lien_presenter(self,function,nb):
        if nb==1:
            ...
            self.demander_nom_equipe=partial(function)

    def gestion_mur_temporaire(self):
        for mur in self.murs:
            if mur.pouvoir==4:
                mur.temps_restant -=1
                if mur.temps_restant==0:
                    for (x,y) in mur.positions:
    ...
    def verification_limite_plateau(self,x,y):
    ...
    def placement_mur_legal(self,x,y,direction,wall_type,player):
        ...
        for i in range(longueur):
            if direction ==0:
                ...
                raise ValueError
            if not self.verification_limite_plateau(newx,newy):
                return False
